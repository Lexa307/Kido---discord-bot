...
const VoiceTateUpdate = (old, member) => {
  ...
  // ignore this channel updates - anti layering voice recording;
  if (old.voiceChannelID === member.voiceChannelID) return;
    ...
    pool.query(`SELECT voicem,voiceh,gifts FROM users WHERE id = '${member.user.id}' LIMIT 1`, (err, rows) => {
      ...
      if (rows[0].voicem >= 60) {
        ...
        if (rows[0].voiceh === 11) {
          ...
          /* Gift can drop a private role for 3/5/7 days |
          premium role for 7 days |
          100-200 server currency; */
